...
from apps.MockServer.response import TYPE_NOT_MATCH, EQUALS, NOT_BETWEEN, STR_NOT_CONTAINS, STR_TOO_LONG, MISS, INVALID, \
    VALID
...
class Validator:
    ...
    @classmethod
    def is_not_long(cls, data, length, response=None):
        if len(data) > length:
...
def domain_server(**kwargs):
    ...
    if data is {}:
        ...
        for key, value in form.items():
            ...
            if long:
                msg = Validator.is_not_equals(value, long, response=invalid.get('long'))
                if msg:
                    return msg

            between = expect.get('between')
            if between:
                msg = Validator.is_not_equals(value, between, response=invalid.get('between'))
                if msg:
                    return msg
        return Validator.valid(response=kwargs.get('valid'))